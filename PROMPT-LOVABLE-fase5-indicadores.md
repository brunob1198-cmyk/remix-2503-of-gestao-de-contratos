# Aplicar UMA migration do SGSST no banco (Fase 5 — última)

Preciso que você execute **apenas** o SQL do bloco no final deste prompt,
diretamente no banco. É a migration `20260820180000_indicadores_sst.sql`.
...
4. Depois de executar, registre a versão em
   ...
   NOTHING`, usando a versão `20260820180000`.
5. Se algo falhar, **pare e me relate o erro** em vez de tentar contornar.

## O que esta migration faz

As taxas da NBR 14280 precisavam de duas pontas que não existiam:

**1. Colunas novas em `public.sgsst_incidentes`** — `dias_perdidos`,
`dias_debitados`, `data_afastamento`, `data_retorno` e `cat_emitida`. Sem dias
perdidos não existe taxa de gravidade. Vem também um trigger
(`check_sgsst_incidente_datas`) que recusa retorno anterior ao afastamento e
afastamento anterior à ocorrência — erro de digitação nessas datas passaria a
produção distorcendo a gravidade sem ninguém notar.

**2. Tabela nova `public.sgsst_hht`** — homens-hora trabalhadas por mês, que é o
denominador das taxas. Com RLS (4 policies), trigger de auditoria e trigger de
integridade de tenant.

Ela tem **dois índices únicos parciais** em vez de um só, de propósito:
`projeto_id` nulo significa "consolidado da empresa", e em Postgres `NULL` não
colide com `NULL` num índice único comum — sem o segundo índice daria para
lançar dois consolidados para o mesmo mês.
...
TABLE`, não há `DELETE`, não há `UPDATE`.

Depende de objetos que já existem: `public.empresas`, `public.projetos`,
`public.sgsst_incidentes`, `public.profiles`, a função
...
WHERE n.nspname = 'public' AND c.relname = 'sgsst_hht';

-- 2) Os dois índices únicos parciais foram criados?
SELECT indexname FROM pg_indexes
WHERE schemaname = 'public' AND tablename = 'sgsst_hht'
  AND indexname LIKE 'uq_%'
ORDER BY indexname;

-- 3) As 5 colunas novas chegaram em sgsst_incidentes?
SELECT count(*) AS colunas_novas
FROM information_schema.columns
WHERE table_schema = 'public' AND table_name = 'sgsst_incidentes'
  AND column_name IN ('dias_perdidos','dias_debitados','data_afastamento','data_retorno','cat_emitida');
```

Esperado:
- `sgsst_hht` com `rls_ligado = true` e `policies = 4`
- dois índices: `uq_sgsst_hht_consolidado_periodo` e `uq_sgsst_hht_projeto_periodo`
- `colunas_novas = 5`
