...
async function verifyAndReconcile(supabase: any, log: any, accessToken: string) {
  ...
  try {
    ...
    if ((!conta_azul_transaction_id || conta_azul_transaction_id === conta_azul_protocolo) && conta_azul_protocolo) {
      ...
      if (protResp && protResp.ok) {
        ...
        if (protData.status === "SUCCESS" || protData.status === "PROCESSED" || protData.status === "COMPLETED") {
          conta_azul_transaction_id = protData.evento_financeiro_id || protData.evento_id || protData.resourceId || protData.id;
    ...
    if (parcela?.baixado || parcela?.liquidado || parcela?.situacao === "PAGO" || parcela?.situacao === "LIQUIDADO" || parcela?.status === "PAID" || parcela?.status === "QUITADO") {
