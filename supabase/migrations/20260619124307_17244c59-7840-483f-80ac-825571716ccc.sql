...
BEGIN
    ...
    producao_mensal AS (
        SELECT
            ...
            SUM(
                CASE
                    WHEN COALESCE(ibm.bdi, il.bdi, mkp.bdi_venda, 0) > 0
                        THEN dp.valor_total / COALESCE(ibm.bdi, il.bdi, mkp.bdi_venda)
                    ELSE dp.valor_total
                END
            ) AS custo_direto_orcado
        FROM diarios_obra d
        ...
        LEFT JOIN item_lpu_bdi_mensal ibm
            ON ibm.item_lpu_id = il.id
           AND ibm.mes_referencia = to_char(date_trunc('month', d.data), 'YYYY-MM')
        GROUP BY 1, 2
    ),
    metricas_base AS (
        SELECT
