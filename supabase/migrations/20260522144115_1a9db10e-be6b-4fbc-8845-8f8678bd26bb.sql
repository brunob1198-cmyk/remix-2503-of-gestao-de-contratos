CREATE OR REPLACE FUNCTION public.get_bi_analise_obras()
 RETURNS TABLE("Referência" text, "Área" text, "Projeto" text, "Cliente" text, "Produção (POC)" float8, "% Impostos" float8, "Receita Líquida" float8, "MO" float8, "Mat." float8, "Transp." float8, "Indir." float8, "Custo Direto Real" float8, "Custo Direto Orçado" float8, "Resultado Direto" float8, "Gerência Real" float8, "Gerência Orçada" float8, "Resultado Gerência" float8, "% Gerência Real" float8, "% Gerência Orç." float8, "Custo Total Real" float8, "Custo Total Orçado" float8, "Resultado Total" float8, "MB Orç. (R$)" float8, "MB Real (R$)" float8, "% MB Orç." float8, "% MB Real" float8, "ID Projeto" uuid, "Mês Num" int4, "Ano" int4)
 LANGUAGE plpgsql
 STABLE
AS $function$
BEGIN
