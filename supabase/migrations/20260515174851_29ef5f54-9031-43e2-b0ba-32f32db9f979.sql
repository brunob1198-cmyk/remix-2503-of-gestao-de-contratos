...
SELECT 
    ...
    COALESCE(pm.custo_direto_orcado_producao * mkp.perc_gerencia - cm.gerencia_real, 0) AS delta_gerencia,
    CASE WHEN pm.custo_direto_orcado_producao > 0 THEN (pm.custo_direto_orcado_producao * mkp.perc_gerencia) / pm.custo_direto_orcado_producao ELSE 0 END AS perc_gerencia_orcada,
    CASE WHEN pm.custo_direto_orcado_producao > 0 THEN COALESCE(cm.gerencia_real, 0) / pm.custo_direto_orcado_producao ELSE 0 END AS perc_gerencia_real,
    ...
    COALESCE((COALESCE(pm.custo_direto_orcado_producao, 0) * (1 + mkp.perc_risco + mkp.perc_gerencia + mkp.perc_treinamento) * (1 + mkp.perc_inflacao)) - cm.custo_total_real, 0) AS resultado_total,
    COALESCE(pm.poc * (1 - imp.perc_total_impostos) - cm.custo_total_real, 0) AS mb_real,
    COALESCE(pm.poc * (1 - imp.perc_total_impostos) - (COALESCE(pm.custo_direto_orcado_producao, 0) * (1 + mkp.perc_risco + mkp.perc_gerencia + mkp.perc_treinamento) * (1 + mkp.perc_inflacao)), 0) AS mb_orcada,
