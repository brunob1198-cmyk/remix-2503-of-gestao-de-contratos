-- Inspecao atrasada passa a existir no painel
--
-- O DEFEITO QUE ISTO CORRIGE
--
-- Roteiro 9.9: criar inspecao com data planejada de ontem e nao executar deveria
-- aparecer como pendente/atrasada no painel. Nao aparecia, por tres motivos que se
-- somavam:
--
-- 1. NENHUMA COMPARACAO COM A DATA. Nem a lista nem o painel olhavam
--    data_planejada. A palavra "atrasada" nao existia no modulo de inspecoes --
--    embora ja existisse para nao conformidade (naoConformidadesVencidas).
--
-- 2. O ALERTA NAO INCLUIA INSPECAO. A funcao sgsst_dashboard_alertas unia ASO, EPI,
--    nao conformidade, PT, incidente e treinamento. Inspecao nao estava lah, entao
--    uma auditoria de campo vencida nao gerava aviso algum.
--
-- 3. inspecoesPendentes CONTAVA UM STATUS INEXISTENTE. Filtrava
--    status IN ('PLANEJADA', 'EM_ANDAMENTO'), mas o CHECK de sgsst_inspecoes
--    so permite ('PLANEJADA','EM_EXECUCAO','CONCLUIDA','CANCELADA'). 'EM_ANDAMENTO'
--    nunca pode existir naquela coluna, entao toda inspecao em execucao ficava fora
--    da conta de pendentes -- o painel subnotificava mesmo sem falar de atraso.
--    O mesmo erro estava em aprEmAndamento, e tambem foi corrigido.
--
-- COMO ESTE ARQUIVO FOI PRODUZIDO
--
-- Gerado a partir de 20260814050000_create_sgsst_dashboard_functions.sql com
-- substituicoes pontuais, e nao redigitado: o resto das duas funcoes e
-- byte-identico ao que esta em producao. Retipar 439 linhas de SQL para mudar
-- quatro pontos e onde se perde uma metrica sem ninguem notar.
