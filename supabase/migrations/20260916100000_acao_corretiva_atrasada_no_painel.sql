-- Acao corretiva atrasada nao aparecia no painel
--
-- O DEFEITO
--
-- Roteiro 10.3: "Por o prazo de uma acao no passado -> Aparece em vermelho / como
-- atrasada NO RELATORIO E NO PAINEL."
--
-- O relatorio cumpria: `acoesAtrasadas`, em src/lib/ncDocumento.ts, separa as
-- acoes em aberto cujo prazo passou, e ainda cobra a parte a acao sem prazo.
--
-- O painel nao. Conferido: a tabela sgsst_nao_conformidades_acoes aparecia ZERO
-- vezes nas duas funcoes do painel. Nem metrica, nem alerta.
--
-- POR QUE ISSO IMPORTA
--
-- Sao dois prazos distintos. A NC tem o seu, cada acao corretiva tem o dela, e a
-- acao pode estourar com a NC ainda dentro do prazo. Nesse caso -- que e o caso
-- comum, porque a acao sempre vence antes -- o painel mostrava a empresa em dia
-- enquanto o trabalho corretivo estava parado.
--
-- Um painel existe para ser a unica tela que alguem olha. Quando ele cala sobre
-- o que o relatorio ja sabe, ele nao esta incompleto: esta dando sossego falso.
--
-- A CORRECAO
--
-- Um bloco novo no UNION dos alertas, com urgencia CRITICA. Uma linha por acao,
-- e nao um contador por NC: quem vai cobrar precisa saber QUAL acao venceu.
--
-- Nao virou metrica (cartao com numero) de proposito. O alerta ja e uma lista
-- acionavel com link para a NC; um contador a mais diria "existem 3" sem dizer
-- quais, e o painel ja tem cartoes demais.
--
-- COMO ESTE ARQUIVO FOI PRODUZIDO
--
-- Gerado a partir de 20260915130000_incidente_em_tratamento_no_painel.sql com um
