-- 1. Atualizar mapeamentos
UPDATE public.mapeamento_categorias_erp 
SET categoria_interna = 'Direto' 
WHERE categoria_interna = 'Indiretos';
...
BEGIN
    ...
    custos_mensais AS (
        SELECT 
            ...
            SUM(CASE WHEN c.categoria_interna = 'Direto' THEN c.valor ELSE 0 END) AS direto_cat,
    ...
    metricas_base AS (
        SELECT
            ...
            COALESCE(cm.direto_cat, 0) AS direto_cat,
    ...
    SELECT
        ...
        (custo_direto_orcado + (custo_direto_orcado * perc_risco) + ((custo_direto_orcado + custo_direto_orcado * (perc_risco + perc_gerencia)) * perc_inflacao) + (custo_direto_orcado * perc_gerencia) + ((custo_direto_orcado + custo_direto_orcado * (perc_risco + perc_gerencia)) * perc_treinamento))::float8 AS "Custo Total Orçado",
        ((custo_direto_orcado + (custo_direto_orcado * perc_risco) + ((custo_direto_orcado + custo_direto_orcado * (perc_risco + perc_gerencia)) * perc_inflacao) + (custo_direto_orcado * perc_gerencia) + ((custo_direto_orcado + custo_direto_orcado * (perc_risco + perc_gerencia)) * perc_treinamento)) - (custo_direto_real + gerencia_real))::float8 AS "Resultado Total",
        ((poc * (1 - perc_impostos)) - (custo_direto_orcado + (custo_direto_orcado * perc_risco) + ((custo_direto_orcado + custo_direto_orcado * (perc_risco + perc_gerencia)) * perc_inflacao) + (custo_direto_orcado * perc_gerencia) + ((custo_direto_orcado + custo_direto_orcado * (perc_risco + perc_gerencia)) * perc_treinamento)))::float8 AS "MB Orç. (R$)",
        ((poc * (1 - perc_impostos)) - (custo_direto_real + gerencia_real))::float8 AS "MB Real (R$)",
        (CASE WHEN (poc * (1 - perc_impostos)) > 0 THEN ((poc * (1 - perc_impostos)) - (custo_direto_orcado + (custo_direto_orcado * perc_risco) + ((custo_direto_orcado + custo_direto_orcado * (perc_risco + perc_gerencia)) * perc_inflacao) + (custo_direto_orcado * perc_gerencia) + ((custo_direto_orcado + custo_direto_orcado * (perc_risco + perc_gerencia)) * perc_treinamento))) / (poc * (1 - perc_impostos)) ELSE 0 END)::float8 AS "% MB Orç.",
        (CASE WHEN (poc * (1 - perc_impostos)) > 0 THEN ((poc * (1 - perc_impostos)) - (custo_direto_real + gerencia_real)) / (poc * (1 - perc_impostos)) ELSE 0 END)::float8 AS "% MB Real",
...
-- 5. Recriar a view view_bi_analise_obras
CREATE OR REPLACE VIEW public.view_bi_analise_obras AS 
 SELECT "Referência",
    ...
    "Mês Num",
    "Ano"
   FROM get_bi_analise_obras() get_bi_analise_obras("Referência", "Área", "Projeto", "Cliente", "Produção (POC)", "% Impostos", "Receita Líquida", "MO", "Mat.", "Transp.", "Direto", "Custo Direto Real", "Custo Direto Orçado", "Resultado Direto", "Gerência Real", "Gerência Orçada", "Resultado Gerência", "% Gerência Real", "% Gerência Orç.", "Custo Total Real", "Custo Total Orçado", "Resultado Total", "MB Orç. (R$)", "MB Real (R$)", "% MB Orç.", "% MB Real", "ID Projeto", "Mês Num", "Ano");

-- Re-grant permissions
GRANT SELECT ON public.view_bi_analise_obras TO authenticated;
GRANT SELECT ON public.view_bi_analise_obras TO anon;
GRANT ALL ON public.view_bi_analise_obras TO service_role;
