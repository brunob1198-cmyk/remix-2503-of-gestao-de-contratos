...
export function AnotacoesCampoDialog({
  ...
  const dialogDate = diarioCampo?.data || selectedDate;
      ...
      <DialogTrigger asChild>
        <Button variant="outline" size="sm">
          ...
          <Badge variant="secondary" className="ml-2 text-xs">
            {diarioCampo ? `${fotosCampo.length} foto${fotosCampo.length !== 1 ? "s" : ""}` : "Sem registros"}
          </Badge>
        </Button>
      </DialogTrigger>
        ...
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            <Badge variant="outline" className="ml-2 text-xs">
              {format(new Date(dialogDate + "T12:00:00"), "dd/MM/yyyy")}
            </Badge>
          </DialogTitle>
        </DialogHeader>
